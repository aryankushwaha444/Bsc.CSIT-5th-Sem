...
j=i+1;
raDo[i]=(float)j/n;
raDominus[i]=(float)i/n;
Dplus[i]=raDo[i]-numbers[i];
Dminus[i]=numbers[i]-raDominus[i];
}
}
...
for(i=0;i<n;i++)
{
cout<<setw(10)<<setprecision(2)<<raDo[i];
}
cout<<endl;
cout<<setw(10)<<"D+";
for(i=0;i<n;i++)
...
int main() //main funcDon
{
KS ks1; //object of KS class
ks1.getdata(); //funcDon calls
ks1.BubbleSort();
ks1.calculate();
ks1.display();
ks1.conclusion();
